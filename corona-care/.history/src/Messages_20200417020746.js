...
class Messages extends Component {
  ...
  fetchConvos = () => {
    ...
    fetch(`http://localhost:3000/convos/${this.props.user.id}`, {
      ...
      .then(this.state.activeConvoID ? this.handleConvoClick() : null);
  };
  ...
  handleConvoClick = () => {
    ...
    console.log("this.state.convos before filteing through", this.state.convos);
    var test = this.state.convos.filter(
    ...
    // );
    console.log(
      "convos that are being filtered through to find updated messages",
      this.state.convos
    );
    this.setState(
      {
      ...
      console.log(
        "what the state of activeConversation should be:",
        this.state.convos.filter(convo => convo.id == this.state.activeConvoID)
      ),
  ...
  messagesDisplay = () => {
    console.log(
      "hit messagesDisplay- this means that the state is being reset after clicking respond"
    );
