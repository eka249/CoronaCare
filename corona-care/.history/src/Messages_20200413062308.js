...
class Messages extends Component {
  ...
  handleConvoClick = e => {
    ...
    console.log(
      "right before filtering though theMessages: this.state.convos[0].full_conversation"
    );
    console.log(this.state.convos[0].full_conversation[0]);
    console.log(
      "this.state.activeConvo right before filtering on this condition",
      this.state.activeConvo
    );
    let theMessages = this.state.convos.filter(
      convo => convo.id == this.state.activeConvo
    );
    console.log("right after creating theMessages: theMessages", theMessages);
    console.log(
      "right after filtering though theMessages: theMessages[0].full_conversation"
    );
    console.log(theMessages[0].full_conversation[0]);
