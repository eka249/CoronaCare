...
class RespondToRequest extends Component {
  ...
  handleRespond = () => {
    fetch("http://localhost:3000/convos", {
      ...
      .then(response => this.handleMessage(response[0]));
  };

  handleMessage = response => {
    console.log("response going to handle message", response);
    fetch("http://localhost:3000/messages", {
      ...
      .then(resp => resp.json())
      .then(this.setState({ ...this.state, open: !this.state.open }))
      .then(this.props.closeResponseModal());
